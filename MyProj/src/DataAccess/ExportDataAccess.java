...
{
	public static boolean saveExport(LedgerModel ledger) throws SQLException
	{
		Connection cnn = DataConnection.connect();
		String saveExportQuery = "insert into ledger(transaction_date,source,iinvoice_no,iinvoice_date,count,unit_price,total_cost,used,stock_in_hand,sign,remark,item_code,period_id) values(?,?,?,?,?,?,?,?,?,?,?,?,?)";
		PreparedStatement stmt =cnn.prepareStatement(saveExportQuery);
